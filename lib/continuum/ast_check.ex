defmodule Continuum.AstCheck do
  ...
  @forbidden %{
    ...
    {NaiveDateTime, :utc_now} => "use Continuum.now() |> DateTime.to_naive()",
    ...
    {Enum, :random} => "derive a deterministic index with Continuum.random/0",
    {Enum, :take_random} => "select deterministically with Continuum.random/0",
    {Enum, :shuffle} => "order deterministically with Continuum.random/0",
    ...
    {Application, :get_env} => "read application config at workflow start; pass it as input",
    {Application, :fetch_env} => "read application config at workflow start; pass it as input",
    {Application, :fetch_env!} => "read application config at workflow start; pass it as input",
    {Application, :get_all_env} => "read application config at workflow start; pass it as input",
    {Node, :list} => "cluster topology is non-deterministic; wrap in an activity",
    {Node, :self} => "pass node placement as workflow input or read it in an activity",
    {Node, :alive?} => "node placement is non-deterministic; wrap in an activity",
    {Node, :connect} => "cluster topology is non-deterministic; wrap in an activity",
    {Node, :disconnect} => "cluster topology is non-deterministic; wrap in an activity",
    {Node, :monitor} => "cluster topology is non-deterministic; wrap in an activity",
    {Node, :monitor_nodes} => "cluster topology is non-deterministic; wrap in an activity",
    {Node, :ping} => "cluster topology is non-deterministic; wrap in an activity",
    {Node, :spawn} => "use await(child(MyWorkflow.run(input))) for durable concurrent work",
    {Node, :spawn_link} => "use await(child(MyWorkflow.run(input))) for durable concurrent work",
    ...
    {Process, :send_after} => "use timer(milliseconds)",
    {Process, :sleep} => "use timer(milliseconds)",
    {Process, :spawn} => "use await(child(MyWorkflow.run(input))) for durable concurrent work",
    {Process, :get} => "the process dictionary is not durable; pass state explicitly",
    {Process, :put} => "the process dictionary is not durable; pass state explicitly",
    {Process, :delete} => "the process dictionary is not durable; pass state explicitly",
    {Process, :get_keys} => "the process dictionary is not durable; pass state explicitly",
    {Process, :info} => "process-local state is non-deterministic; wrap in an activity",
    {Process, :list} => "process-local state is non-deterministic; wrap in an activity",
    {Process, :whereis} => "process registration is node-local; wrap in an activity",
    {Process, :registered} => "process registration is node-local; wrap in an activity",
    {Process, :register} => "process registration is node-local; wrap in an activity",
    {Process, :unregister} => "process registration is node-local; wrap in an activity",
    {Process, :alive?} => "process liveness is non-deterministic; wrap in an activity",
    {Process, :group_leader} => "process placement is non-deterministic; wrap in an activity",
    {Process, :flag} => "process flags are not durable; wrap in an activity",
    {Process, :monitor} => "process monitoring is not durable; wrap in an activity",
    {Process, :demonitor} => "process monitoring is not durable; wrap in an activity",
    {Process, :link} => "process links are not durable; wrap in an activity",
    {Process, :unlink} => "process links are not durable; wrap in an activity",
    ...
    {Logger, :warning} => "use Continuum.log(:warning, message)",
    {Logger, :warn} => "use Continuum.log(:warning, message)",
    {Logger, :error} => "use Continuum.log(:error, message)",
    {Logger, :critical} => "use Continuum.log(:critical, message)",
    {Logger, :alert} => "use Continuum.log(:alert, message)",
    {Logger, :emergency} => "use Continuum.log(:emergency, message)",
    {Logger, :log} => "use Continuum.log(level, message)",
    {Logger, :bare_log} => "use Continuum.log(level, message)",
    {Logger, :metadata} => "Logger metadata is process-local; pass context as workflow input",
    {Logger, :reset_metadata} =>
      "Logger metadata is process-local; pass context as workflow input",
    {Logger, :put_process_level} =>
      "Logger process levels are not durable; configure them outside the workflow",
    ...
    {:erlang, :get} => "the process dictionary is not durable; pass state explicitly",
    {:erlang, :put} => "the process dictionary is not durable; pass state explicitly",
    {:erlang, :erase} => "the process dictionary is not durable; pass state explicitly",
    {:erlang, :get_keys} => "the process dictionary is not durable; pass state explicitly",
    {:erlang, :process_info} => "process-local state is non-deterministic; wrap in an activity",
    {:erlang, :processes} => "process-local state is non-deterministic; wrap in an activity",
    {:erlang, :whereis} => "process registration is node-local; wrap in an activity",
    {:erlang, :registered} => "process registration is node-local; wrap in an activity",
    {:erlang, :register} => "process registration is node-local; wrap in an activity",
    {:erlang, :unregister} => "process registration is node-local; wrap in an activity",
    {:erlang, :is_process_alive} => "process liveness is non-deterministic; wrap in an activity",
    {:erlang, :group_leader} => "process placement is non-deterministic; wrap in an activity",
    {:erlang, :process_flag} => "process flags are not durable; wrap in an activity",
    {:erlang, :monitor} => "process monitoring is not durable; wrap in an activity",
    {:erlang, :demonitor} => "process monitoring is not durable; wrap in an activity",
    {:erlang, :link} => "process links are not durable; wrap in an activity",
    {:erlang, :unlink} => "process links are not durable; wrap in an activity",
    {:erlang, :exit} => "process mutation is non-deterministic; wrap in an activity",
